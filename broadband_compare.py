...
def main(user_data):
    ...
    if(user_data.max_dl_speed):
        print(
            ...
            f"Average download speed of {user_data.location}: {get_value(location, 'avg_dl_speed')}Mb/s\n")

    if(user_data.max_ul_speed):
        print(
            f"Some information about upload speed\n"
        )
...
def parse_args():
    ...
    parser.add_argument("-l", "--location", dest="location", help="The location to perform internet "
                                                                  "analysis. E.g. Leicester", type=str)
    parser.add_argument("-d", "--max_download_speed", dest="max_dl_speed", help="Maximum download speed Mb/s "
                                                                                "this connection e.g 50")
    parser.add_argument("--min_download_speed", dest="min_dl_speed", help="Minumum speed in Mb/s.")
    parser.add_argument("-u", "--max_upload_speed", dest="max_ul_speed", help="Maximum upload speed in Mb/s")
    parser.add_argument("--min_upload_speed", dest="max_ul_speed", help="Maximum upload speed in Mb/s")
